  ...
  return (
    <DashboardLayout>
      <div className="flex gap-8 p-6 sm:p-8 md:p-10">
        ...
        <div className="flex-1 bg-white shadow rounded-md px-6 sm:px-10 py-8">
          <div className="block md:hidden mb-12">
            <div className="flex gap-6 border-b">
              <div>
                <Link to={'/settings'} onClick={() => handleLinkClick('/settings')} className={`flex items-center gap-4 px-4 py-2.5 cursor-pointer ${activeLink === '/settings' ? "border-b-2 border-violet-600" : "border-0"}`}>
                  <img src={userIcon} alt="" />
                  <span className="text-zinc-700 text-sm">Account Details</span>
                </Link>
              </div>
              <div>
                <Link to={'/settings/security-settings'} onClick={() => handleLinkClick('/settings/security-settings')} className={`flex items-center gap-4 px-4 py-2.5 cursor-pointer ${activeLink === '/settings/security-settings' ? "border-b-2 border-violet-600" : "border-0"}`}>
                  <img src={lock} alt="" />
                  <span className="text-zinc-700 text-sm">Security</span>
                </Link>
              </div>
